...
<prereqs><exclusives>}
'''
translator = deepl.Translator("51fed646-552e-31d8-ccd1-4346a1e91184:fx")
...
class App(tk.Frame):
	...
	def run_app(self):
		...
		for e in BOX_REGEX.findall(i_contents):
			...
			#else:
			trsText = text
			translatedText = translator.translate_text(trsText, target_lang="EN-US")
			print(translatedText.text)
			focus_name = text
			
			focus_tag = re.sub(NON_ALPHANUMERIC_REGEX, '', translatedText.text.replace(' ', '_').lower())
